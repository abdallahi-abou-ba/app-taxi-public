...
- Mise en relation chauffeur ↔ client en temps réel (Socket.IO) : le chauffeur disponible le plus proche reçoit la demande
...
- Calcul de distance/durée/tarif estimé, trajet réel affiché sur la carte (OSRM)
...
- Tests automatisés : **Jest + Supertest**, 10 fichiers de tests couvrant l'auth, les courses, le chat, les paiements, les notations, le parrainage, les rappels, l'historique, les réservations et la suppression de compte — exécutés sur une base de données dédiée aux tests

Modèle de données principal : `User` (rôle, disponibilité et position en direct pour les chauffeurs, note moyenne, parrainage, crédit), `Ride` (cycle de vie complet, tarif, trajet), `Message` (chat lié à une course), `RefreshToken` (sessions révocables).
...
- **socket.io-client** pour le temps réel (position du chauffeur, nouvelles demandes, statut de course, chat)
...
### En cours
- Appels vocaux en direct pendant une course (WebRTC) — nécessite un build de développement EAS, car Expo Go ne supporte pas les modules natifs WebRTC
...
- Modération et signalement des messages dans le chat
